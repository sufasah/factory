...
const DEFAULT_DATE = ''
const DEFAULT_TIME = '00:00'//'00:00:00'
...
export default {
  ...
  computed: {
    ...
    formattedDatetime() {
      var res = this.selectedDatetime ? format(this.selectedDatetime, this.dateTimeFormat) : ''
      return res;
    },
    selectedDatetime() {
    ...
    dateSelected() {
      this.formattedDatetimeChange(this.date)
      this.testKey++
      return !this.date
    }
  },
  methods: {
    ...
    okHandler() {
      if(!this.selectedDatetime || !this.checkChooseable(this.selectedDatetime))
        return
  ...
  watch: {
    ...
    time: function(val){
      if(this.lastTime.substring(2,5) == val.substring(2,5)){
        this.hourChange(val.substring(0,2))
      }
      this.lastTime = val
    }
  }
